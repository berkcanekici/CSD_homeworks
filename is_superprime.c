...
int main() {
    ...
    printf("Super asalligini test etmek istediginiz sayiyi giriniz.\n");
    scanf("%d", &ival);
   
    if (!is_prime(ival))
        printf("%d bir super asal sayi degildir", ival);
    else
        is_superprime(ival);
}
...
int is_superprime(int val) {
    ...
    if(is_prime(count))
        printf("%d bir super asal sayidir.", val);
    else
        printf("%d bir super asal sayi degildir.", val);
